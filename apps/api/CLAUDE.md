...
- **Database**: PostgreSQL (Render) — Hibernate `ddl-auto: update` + `schema.sql` init script. `ddl-auto=update` is unreliable for anything beyond adding a column/table — it never updates check constraints (see `scripts_kind_check`/`secrets_auth_type_check` in `schema.sql`) and, worse, it can generate **invalid SQL that fails silently on every boot**: `@Column(columnDefinition = "varchar(32) default 'CONNECTED'")` on `SecretEntity.status` produced `ALTER COLUMN status SET DATA TYPE varchar(32) default 'CONNECTED'` whenever Hibernate decided the column's type needed changing — `DEFAULT` isn't valid syntax on a `SET DATA TYPE` clause, so it threw `CommandAcceptanceException`/`PSQLException: syntax error at or near "default"` on every startup, caught by Spring's schema-tool error handling and silently ignored (the app boots fine either way). Found by grepping `Position: N` from the Postgres error against every `columnDefinition` containing `default` in the codebase, computed against the exact log text **with** `hibernate.format_sql=true` pretty-printing (the raw one-liner gives the wrong position). Fix: never put `DEFAULT` inside a JPA `columnDefinition` that Hibernate might ever try to `ALTER ... TYPE` — set the type only there, and add the `DEFAULT` as a separate idempotent `ALTER TABLE ... ALTER COLUMN ... SET DEFAULT ...` in `schema.sql`, same as the check-constraint pattern.
