...
- **Secrets are encrypted at rest in the backend** (AES-256-GCM, `SecretCipher.kt`, master key via `SECRETS_MASTER_KEY`) — not just a local vault. `GET /secrets/{id}/value` resolves the decrypted value, scoped by `@GetUserId`, called only by the runner (never the browser). `apiKeyRef`/`authRef` reference a `Secret.id` (not a label). `electron/secrets-vault.ts` is now only an optional offline read-through cache (`SecretCache`) if the backend is briefly unreachable — the backend is the source of truth. A single `resolveSecret`/`applyAuthToHttpTarget` path in `runner.ts` covers both the LLM provider's `apiKeyRef` and tools' `authRef`, across 8 `authType` schemes (bearer/header/query/basic/oauth2_client_credentials/oauth2_refresh/oauth1/raw) — see plan doc §8.
- **OAuth 1.0a (`.specs/003-autenticacao-oauth1-nas-ferramentas`)** — the one scheme whose header can't be calculated once and reused: it's a signature over method + final URL + a per-request nonce/timestamp (RFC 5849), so `applyAuthToHttpTarget` returns an `oauth1?: OAuth1Credentials` object instead of a header, and whoever owns the final URL signs at call time (D2/D11) — the tool-calling loop (`openai-tools.ts`'s `buildHttpTool`, right before `fetch`) and the separate `http-tool.mjs` MCP process (from `WORKESTRATOR_HTTP_TOOL_CONFIG`), never `runner.ts` itself. `electron/mcp-servers/oauth1.mjs` is a byte-faithful JS port of the Kotlin `OAuth1Signer` (`apps/api`) — **it must keep producing identical signatures to the Kotlin side for the same input** (RF11); both are verified against the same test vectors (RFC 5849 §3.4.1.3.1 normative example + a real Fluig URL with a non-default port). Changing the percent-encoding or URL-normalization logic on one side without mirroring the other is a silent divergence — 401 on one executor, success on the other. Two paths can't support it at all and refuse to build the integration instead of signing once and reusing (D10): the MCP HTTP server path (`buildMcpServerEntry`, `transport: "http"`) and the connector gateway path — both hand out a fixed header set reused across N calls, which a per-request signature can't fit into. **Every consumer of `applyAuthToHttpTarget` that then makes a request MUST apply the signer** — destructuring only `{ headers, url }` drops the `oauth1` field silently and the call goes out with no `Authorization`, which a legacy API answers with a bare 401. This already happened once: `handleTestTool` (the tool's "Testar" button, which makes a real request) shipped without it and produced a `401 FDNUnauthenticatedAccessDeniedException` against Fluig while the whole test suite stayed green — no test exercised that path, and tests that call the signer directly pass even with the bug present. The exhaustive consumer list lives in the spec's design doc (`C:\Projetos\docs\Workestrator\003-.../design.md`, §Consumidores que precisam assinar); the type does **not** enforce this, so a new consumer can reintroduce the same bug. Note also that "Testar conexão" on the *secret* (`handleTestSecret`) does **not** validate oauth1 signing at all — for this scheme it only confirms a value is stored.
...
- **Scheduler local**: `orchestrator-shared/runtime/scheduler.ts`, mounted via `<OrchestratorScheduler />` in `security/layout.tsx`. Polls every 30s for squads with `trigger: { type: "schedule", enabled: true }` and fires `startRun(squadId, briefing, "schedule")`. A cadência é `every: "5m" | "1h" | "daily"` (intervalo fixo desde o último disparo) ou `every: "custom"` + `rrule`, uma **RRULE da RFC 5545** avaliada por `runtime/rrule.ts` (lib `rrule`). A regra é interpretada em **hora local** — a lib trabalha em UTC sem `tzid`, então `rrule.ts` converte os instantes pro "domínio flutuante" (componentes locais reempacotados como UTC) antes de calcular e desfaz no resultado; mexer nessa conversão desloca todo disparo pelo offset do fuso. `parseRrule` só passa a âncora `dtstart` quando o texto **não** traz `DTSTART` próprio (a opção sobrescreve o do texto, o que jogaria fora a âncora que o usuário escreveu). RRULE ausente ou inválida **não dispara** em vez de cair num intervalo padrão. `describeRrule` só descreve `BYDAY`/`BYHOUR` que o usuário digitou — a lib preenche esses campos a partir do `dtstart` e repetir isso descreveria o instante de criação como se fosse escolha do usuário. Mesmas funções alimentam o preview de próximas execuções no `squad-form-dialog` (`rrule-field.tsx`), pra a UI mostrar exatamente o que o scheduler vai disparar. Runs in the **renderer** (not the Electron main process) — deliberately, to reuse `orchestrator-runtime.ts`'s module-level engine instead of inventing a new main→renderer IPC channel. Only fires while the app window is open.
- **Aprovação externa de checkpoint (n8n → Teams) + aprovador delegado** (`.specs/001-aprovacoes-externas-teams`): `enterCheckpoint` (`orchestrator-runtime.ts`) transiciona pro estado `checkpoint` na hora (nunca bloqueia o run) e só **depois** cria o `ApprovalRequest` no backend (`POST /approvals`, `features/security/approvals/api`) — se o POST falhar, o checkpoint segue decidível só localmente (`settleCheckpointLocally`). Com `pendingApprovalId` registrado, `resolveCheckpoint` decide via backend (`POST /approvals/{id}/decide`) e converge em `settleCheckpoint` — a mesma função que `runtime/approval-watcher.ts` chama ao detectar uma decisão externa (poll incondicional de 10s enquanto há `pendingApprovalId`, design D7: quem está rodando o squad pode não ser quem decide — aprovador delegado, ou o dono em outra sessão/dispositivo). `AgentApprovalPolicy` (`approverUserIds`/`ownerCanDecide`) é resolvida no servidor a partir do agente **no momento da criação do pedido** (snapshot, D8) — mudar a política depois não muda quem pode decidir um pedido já pendente. Reprovar sem justificativa é bloqueado no próprio `resolveCheckpoint`, não só na UI — é a entrada do treinamento pós-reprovação (`.specs/002-treinamento-pos-reprovacao`, `RunRecord.rejections[]`).
- **Aprovação por item (design D15/D16)**: quando o artefato do passo anterior é uma **lista JSON**, `enterCheckpoint` extrai os itens com `runtime/approval-items.ts` (`parseApprovalItems`, módulo puro e testável) e cria o pedido com `items[]` — cada chamado/post/pedido ganha veredito próprio, e o `summary` passa a ser um **resumo legível** (`summarizeApprovalItems`) em vez do JSON cru, que o teto de 500 chars do backend cortava no meio da palavra. A extração é **só fallback**: saída em prosa ⇒ `[]` ⇒ checkpoint booleano de sempre, então nenhum squad existente muda de comportamento. **Um objeto JSON solto conta como lista de um** — modelo que encontrou exatamente um chamado devolve `{…}` em vez de `[{…}]` com frequência, e exigir o array fazia o lote de um item cair no booleano com o JSON inteiro no `summary` (truncado em 500 chars pelo backend) e `items: []` no payload do n8n. Array antes de objeto solto na ordem de tentativa, para `{"chamados": [ … ]}` render a lista de dentro. `ref`/`label` saem de **heurística genérica** (regex de `num|id|processo|…` e `nome|titulo|necessidade|…`), nunca de campo fixo — o mesmo checkpoint serve a qualquer domínio, e a tela renderiza `item.data` como pares chave/valor sem conhecer nome de campo. `ApprovalItemsPanel` (`features/security/approvals/components`) é compartilhado entre a tela dedicada de decisão e o `run-interaction-panel`, para a regra de "o que ainda é decidível" ser idêntica nos dois. Ao fechar o último item, o painel chama `applyApprovalDecision` para adiantar o run — sem isso o `approval-watcher` levaria até 10s e pareceria travado; é seguro chamar em paralelo porque `settleCheckpoint` ignora quem chega depois. Reprovações por item viram **uma `RunRejection` cada**, com `itemRef` preenchido (o treinamento de 002 precisa saber *qual* item falhou) — inclusive no caso misto, em que o pedido é aprovado mas alguns itens foram reprovados. **E o item reprovado sai do contexto do próximo passo** (D16: "aprovado ⇒ segue com o subconjunto aprovado"): `settleCheckpoint` chama `applyRejectedItemsFilter`, que guarda em `contextOverrides` (por execução, keyed por `stepId`, ou pelo briefing quando o checkpoint `"before"` abre antes do primeiro passo) a lista reescrita por `stripRejectedApprovalItems`. Todo prompt lê o run pela `contextView` — agent (`runOrchestratedAgentStep`) e coordenador (`buildCoordinatorPrompt`) —, então vale nos dois kinds; filtrar só no `"before"` deixava o vazamento andar um passo pra frente pelo `advanceOrchestrated`. É **override de contexto, nunca reescrita do artefato**: o `RunRecord` persistido é auditoria e as `RunRejection` de 002 apontam pro conteúdo original — por isso o override vive só em memória (`continueRun` re-liquida o checkpoint e o filtro é recalculado antes da continuação). Filtra por **exclusão** dos `rejected` (nunca por inclusão dos `approved` — `pending` é estado válido e sumiria), e quando não dá pra alinhar com segurança (artefato que não é mais a lista de origem, ou rabo acima de `MAX_APPROVAL_ITEMS` que nunca chegou ao pedido) o log **diz** o que ficou de fora em vez de cortar calado. Relacionado: `Runtime.pendingContextSteps` guarda o `context_steps` que o coordenador escolheu antes de um checkpoint `"before"` — sem isso a aprovação descartava a escolha e o agent caía no fallback "só o passo anterior", ou seja, **ter** checkpoint estreitava o contexto em relação a não ter.
