...
function MotionHighlight<T extends string>({ ref, ...props }: MotionHighlightProps<T>) {
	...
	const parentModeProps = props as ParentModeMotionHighlightProps;
	const boundsOffset = parentModeProps.boundsOffset;

	const safeSetActiveValue = (id: string | null) => {
		const typedId = id as T | null;
		let changed = false;

		setActiveValue((prev) => {
			if (prev === typedId) return prev;
			changed = true;
			return typedId;
		});

		if (changed) onValueChange?.(typedId);
	};
	...
	}, [mode, activeValue, boundsOffset?.top, boundsOffset?.left, boundsOffset?.width, boundsOffset?.height]);
		...
		if (mode === "parent") {
			return (
				<div
					...
					className={cn("relative", parentModeProps.containerClassName)}
				>
	...
	return (
		<MotionHighlightContext.Provider
			value={{
				...
				forceUpdateBounds: parentModeProps.forceUpdateBounds,
			}}
		>
			{enabled
				? controlledItems
