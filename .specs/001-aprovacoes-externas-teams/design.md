...
| D5 | Tela de decisão **dedicada** (`/dashboard/aprovacoes/:id`), fora da árvore de rotas do squad (a rota segue fora; o que ela mostra foi ampliado por **D17**) | o aprovador não deve alcançar nada do squad além do que a aprovação carrega; uma rota nova com guarda de autorização própria é mais simples de auditar que remendar exceções nas rotas do squad | reaproveitar `page-squad-detail`/`run-dialog` com um branch de permissão — haveria dezenas de pontos onde vazar dados do squad por descuido |
...
| D17 | O participante de um pedido **vê a execução**, por uma rota pendurada no pedido (`GET /approvals/:id/run`), nunca pelas rotas do squad | D5 mantinha o aprovador cego: ele decidia sobre um resumo de texto e, depois de aprovar, não tinha como acompanhar o que o squad fez com aquilo. Corrigir isso afrouxando `RunService.getRunForUser`/`SquadService.getSquadForUser` transformaria o aprovador em quase-dono de tudo do squad — o vazamento que D5 existe para evitar. Pendurar a leitura no pedido preserva a regra: a autorização é a mesma de `get` (`canView`), o par `runId`/`squadId` vem do pedido e não do cliente, e o recorte devolvido (`ApprovalRunResponse`) omite conexões, reprovações e qualquer configuração de agente. `listByRun` passa a filtrar por `canView` pelo mesmo motivo — filtrar por dono devolvia lista vazia justamente para quem tinha acabado de decidir | relaxar as rotas de run/squad para aceitar aprovador (dezenas de pontos de vazamento, exatamente a alternativa que D5 já descartou); ou manter o aprovador cego (a decisão fica pior, e o acompanhamento pós-decisão fica impossível) |
